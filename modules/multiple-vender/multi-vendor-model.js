...
import { getProductoById } from "../productos/model.js";
...
export async function procesarCompraMultiVendedor(id_usuario, productosCarrito, id_ubicacion) {
  ...
  try {
    ...
    if (vendedores.length === 1) {
      // 3a. Un solo vendedor - crear pedido simple
      ...
      for (const producto of grupo.productos) {
        await client.query(
          `INSERT INTO pedido_producto (id_pedido, id_producto, cantidad, precio) 
           VALUES ($1, $2, $3, $4)`,
          [pedidoSimple.id_pedido, producto.id, producto.cantidad || 1, producto.precio]
        );
      }
      ...
      // 3b. Múltiples vendedores - crear pedido maestro y sub-pedidos
      ...
      for (const vendedorId of vendedores) {
        ...
        // Crear productos del sub-pedido
        for (const producto of grupo.productos) {
          await client.query(
            `INSERT INTO pedido_producto (id_pedido, id_producto, cantidad, precio) 
             VALUES ($1, $2, $3, $4)`,
            [subPedido.id_pedido, producto.id, producto.cantidad || 1, producto.precio]
          );
        }
